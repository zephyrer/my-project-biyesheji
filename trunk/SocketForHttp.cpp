// SocketForHttp.cpp: implementation of the CSocketForHttp class.
//
//////////////////////////////////////////////////////////////////////
...
BOOL CSocketForHttp::Connect(char *szHostName,int nPort)
{
	if(szHostName==NULL)
		return FALSE;
	...
	*/
	int returnCode=connect(m_s,(struct sockaddr*)&destaddr,sizeof(destaddr));
	if(returnCode!=0)
	{
		//CloseSocket();
		//m_s=NULL;
...
///根据请求的相对URL输出HTTP请求头
const char *CSocketForHttp::FormatRequestHeaderWithHead(char *pServer,char *pObject, long &Length,
									  ...
									  long nTo,int nServerType)
{
	...
	memset(m_requestheader,'\0',2048);

	///第1行:方法,请求的路径,版本
	strcat(m_requestheader,"HEAD ");
...
///根据请求的相对URL输出HTTP请求头
const char *CSocketForHttp::FormatRequestHeader(char *pServer,char *pObject, long &Length,
									  char *pCookie,char *pReferer,long nFrom,
									  long nTo,int nServerType)
{
	char szPort[10];
...
///发送请求头
BOOL CSocketForHttp::SendRequest(const char *pRequestHeader, long Length)
{
	if(!m_bConnected)return FALSE;
...
long CSocketForHttp::Receive(char* pBuffer,long nMaxLength)
{
...
///关闭套接字
BOOL CSocketForHttp::CloseSocket()
{
	if(m_s != NULL)
	{
...
int CSocketForHttp::GetRequestHeader(char *pHeader, int nMaxLength) const
{
	int nLength;
...
//设置接收或者发送的最长时间
BOOL CSocketForHttp::SetTimeout(int nTime, int nType)
{
	if(nType == 0)
	{
		nType = SO_RCVTIMEO;
...
// 	int len;
// 	len=sizeof(val);
// 	int b=getsockopt(m_s,SOL_SOCKET,SO_SNDBUF,(char*)&val,&len);
	return TRUE;
}

//获取HTTP请求的返回头
const char* CSocketForHttp::GetResponseHeader(int &nLength)
{
	if(!m_bResponsed)
	{
		char c = 0;
...
// 				bEndResponse = TRUE;
// 			}
			if (m_ResponseHeader[nIndex - 1] == '\n'&&m_ResponseHeader[nIndex - 4] == '\r')
			{
				bEndResponse = TRUE;
			}
		}
...
//返回HTTP响应头中的一行.
int CSocketForHttp::GetResponseLine(char *pLine, int nMaxLength)
{
...
int CSocketForHttp::GetField(const char *szSession, char *szValue, int nMaxLength)
{
	//取得某个域值
...
int CSocketForHttp::GetServerState()
{
	//若没有取得响应头,返回失败
	if(!m_bResponsed) return -1;
